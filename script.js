...
// EXPORTAÇÃO DO PERCURSO
// ----------------------------------------------------------------------
// Exporta o caso, percurso detalhado e tentativas como arquivo JSON no navegador.
$("export").onclick = () => {
  const data = {
    case:
      activeCase === "marina"
        ? "Marina, 23 anos — dor abdominal e vômitos"
        : "João, 54 anos — dor torácica",
    exportedAt: new Date().toISOString(),
    // Enriquece cada passo exportado com sua posição e os dados do catálogo clínico.
  ...
  downloadLink.download = "percurso-paciente-virtual.json";
  downloadLink.click();
  // Libera a URL temporária após iniciar o download.
  setTimeout(() => URL.revokeObjectURL(url), 1000);
  notify("Percurso exportado.");
};
  ...
  if (selected === "marina") {
    ...
    footer.innerHTML =
      ...
      marinaCase.references
        ...
        .join(" e ") +
      ". O percurso fica nesta sessão; use “Exportar percurso” ao concluir.";
  }
  render();
